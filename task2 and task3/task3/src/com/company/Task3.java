...
public class Task3 {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        CabinMenu(room, guest,queuelist);
    }

    public static void CabinMenu(Cabin[][] room, Passenger guest,Queuelist queuelist) throws FileNotFoundException {
        ...
        while (true) {
            ...
            if (selection.equals("A")) {
                ...
                AddCustomerCabin(room, guest, queuelist);
                ...
                DeleteCabins(room, guest, queuelist);
    ...
    public static void AddCustomerCabin(Cabin[][] room, Passenger guest,Queuelist queuelist) {
        Scanner input = new Scanner(System.in);
        int i=0;
        int count=12;
        while (i<12){
            if ((room[i][0].getGuest()!=guest) ||(room[i][1].getGuest()!=guest) || (room[i][2].getGuest()!=guest)){
                count--;
            }
            i++;
        }
        if (count==0){
            System.out.println("All cabins are full...adding to the queue");
            System.out.println("Enter firstname: ");
            String firstname = input.next();
            System.out.println("Enter surname: ");
            String surname = input.next();
            queuelist.addQueuelist(new Passenger(firstname,surname,1000));
    ...
    public static void DeleteCabins(Cabin[][] room, Passenger guest, Queuelist queuelist) {
        Scanner input = new Scanner(System.in);
        System.out.println("Enter your passenger number: ");
        int num = input.nextInt();
        System.out.println("Enter your cabin number: ");
        int num2 = input.nextInt();
        if (queuelist.emptyQueue()){
            room[num2][num].setGuest(guest);
        }else {
            Passenger guestqueue = queuelist.deleteQueuelist();
            room[num2][num].setGuest(guestqueue);
        }
    ...
    public static void StoreData(Cabin[][] room, Passenger guest) throws FileNotFoundException {
        ...
        for (int x = 0; x < 12; x++) {
            for (int y = 0; y < 3; y++) {
                file.println("cabin no-" + (x+1) + " Passenger no- " + (y+1) + room[x][y].getGuest());
            }
        }
    ...
    public static void LoadData(Cabin[][] room, Passenger guest) throws FileNotFoundException {
        File file = new File("C:\\Users\\DELL\\Desktop\\task3\\StoreCabinData2.txt");
    ...
    public static void TotalExpenses(Cabin[][] room, Passenger guest){
        ...
        for (int x = 0; x < 12; x++) {
            for (int y = 0; y < 3; y++) {
                int expense = room[x][y].getGuest().getExpenses();
                if (expense>0){
